...
/** 生成 N 个测试邀请码 */
function makeCodes(n: number, overrides: Record<string, unknown> = {}): Record<string, unknown>[] {
...
beforeEach(() => {
  ...
  vi.stubGlobal("fetch", mockFetch.mockResolvedValue(makeAuthResponse(makeCodes(5))));
});
...
describe("AC-6: REVOKED 过滤", () => {
  ...
  it("total 应基于过滤后的数量（不含 REVOKED）", async () => {
    const codes = [...makeCodes(3), ...makeCodes(2, { status: "REVOKED" })];
    mockFetch.mockResolvedValue(makeAuthResponse(codes, { used: 5, total: 5 }));
  ...
  it("hasMore 基于过滤后的总数计算", async () => {
    const codes = [...makeCodes(4), ...makeCodes(3, { status: "REVOKED" })];
...
describe("AC-7: auto-fill 仅在 offset=0 时触发", () => {
  it("offset=0 且 codes 不足配额时，应触发 generate 调用", async () => {
    ...
    mockFetch
      ...
      .mockResolvedValueOnce(makeAuthResponse(initialCodes, { used: 2, total: 5 }))
      // generate 调用（3 次）
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}) })
      // 第二次 fetchCodes：补充后的 codes
      .mockResolvedValueOnce(makeAuthResponse(filledCodes, { used: 5, total: 5 }));
    ...
    const postCalls = mockFetch.mock.calls.filter(
      (call: unknown[]) => call[1] && (call[1] as Record<string, string>).method === "POST"
    );
  ...
  it("offset>0 时，不应触发 generate 调用", async () => {
    ...
    mockFetch.mockResolvedValue(makeAuthResponse(codes, { used: 2, total: 5 }));
