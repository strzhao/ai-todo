...
describe("parseItem", () => {
  ...
  it("多余字段不出现在结果中", () => {
    const result = parseItem({ title: "t", extra_field: "should_be_ignored" }, "t") as Record<
      string,
      unknown
    >;
...
describe("parseActions - 新格式", () => {
  it("单个 create action，含 tasks 数组", () => {
    const result = {
      actions: [{ type: "create", tasks: [{ title: "任务A" }, { title: "任务B" }] }],
    };
  ...
  it("complete action：正确提取 target_id 和 target_title", () => {
    const result = {
      actions: [{ type: "complete", target_id: "uuid-123", target_title: "调研任务" }],
    };
    const actions = parseActions(result, "");
  ...
  it("update changes.priority 为字符串 → 不出现在 changes 中", () => {
    const result = {
      actions: [{ type: "update", target_id: "id1", changes: { priority: "1" } }],
    };
    const actions = parseActions(result, "");
  ...
  it("update changes.tags 为数组 → 正确提取", () => {
    const result = {
      actions: [{ type: "update", target_id: "id1", changes: { tags: ["工作", "重要"] } }],
    };
    const actions = parseActions(result, "");
  ...
  it("update changes.assignee_email 为 null → 正确提取为 null", () => {
    const result = {
      actions: [{ type: "update", target_id: "id1", changes: { assignee_email: null } }],
    };
    const actions = parseActions(result, "");
      ...
      actions: [
        {
          type: "create",
          tasks: [
            { title: "接口联调", parent_target_id: "uuid-parent", parent_target_title: "项目计划" },
          ],
        },
      ],
    };
  ...
  it("update changes progress 超出范围时被忽略", () => {
    const result = {
      actions: [{ type: "update", target_id: "uuid-1", changes: { progress: 150 } }],
    };
    const actions = parseActions(result, "");
  ...
  it("update changes progress 为 0 时正确保留", () => {
    const result = {
      actions: [{ type: "update", target_id: "uuid-1", changes: { progress: 0 } }],
    };
    const actions = parseActions(result, "");
