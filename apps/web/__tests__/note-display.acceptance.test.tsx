...
describe("NoteCard — Markdown 渲染", () => {
  it("包含 # 标题语法的 title 应渲染为 h1 元素", () => {
    const { container } = render(
      <NoteCard note={makeNote({ title: "# 会议纪要" })} onUpdate={vi.fn()} onDelete={vi.fn()} />
    );
  ...
  it("包含 ## 标题语法的 title 应渲染为 h2 元素", () => {
    const { container } = render(
      <NoteCard note={makeNote({ title: "## 子标题" })} onUpdate={vi.fn()} onDelete={vi.fn()} />
    );
  ...
  it("包含加粗语法的 title 应渲染为 strong 元素", () => {
    const { container } = render(
      <NoteCard note={makeNote({ title: "**重要内容**" })} onUpdate={vi.fn()} onDelete={vi.fn()} />
    );
  ...
  it("纯文本 title 应正常渲染为段落", () => {
    const { container } = render(
      <NoteCard note={makeNote({ title: "普通文本笔记" })} onUpdate={vi.fn()} onDelete={vi.fn()} />
    );
...
describe("NoteCard — 长内容折叠", () => {
  it("短内容不显示展开按钮", () => {
    const { container } = render(
      <NoteCard note={makeNote({ title: "简短" })} onUpdate={vi.fn()} onDelete={vi.fn()} />
    );
