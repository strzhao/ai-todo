...
describe("SpaceTaskNode 接口", () => {
  it("包含 assignee_email 可选字段", () => {
    const interfaceMatch = spaceNavSource.match(/interface\s+SpaceTaskNode\s*\{([^}]+)\}/);
...
describe("SpaceNav 成员数据获取", () => {
  ...
  it("存在成员数据的存储结构（spaceMembersMap 或类似）", () => {
    ...
    const hasMembersStore = /spaceMembersMap|membersMap|membersData|spaceMembers/.test(
      spaceNavSource
    );
...
describe("getDisplayLabel 集成", () => {
  ...
  it("渲染逻辑中调用 getDisplayLabel", () => {
    ...
    const callCount = (spaceNavSource.match(/getDisplayLabel\s*\(/g) || []).length;
...
describe("getDisplayLabel 函数", () => {
  it("display-utils.ts 文件存在并导出 getDisplayLabel", () => {
    ...
    expect(utilsSource).toMatch(/export\s+(function\s+getDisplayLabel|const\s+getDisplayLabel)/);
  });
...
describe("经办人显示：文字标签 vs 首字母圆圈", () => {
  it("渲染逻辑中存在基于成员数据的条件分支", () => {
    ...
    const hasLookup = /\.find\s*\(|\.get\s*\(|members.*assignee_email|assignee_email.*members/.test(
      spaceNavSource
    );
