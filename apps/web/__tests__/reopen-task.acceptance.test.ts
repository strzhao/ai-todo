...
describe("AC-1: ParsedAction type 包含 reopen", () => {
  it("parseActions 解析到 reopen type 后，返回 action.type === 'reopen'", () => {
    const result = {
      actions: [{ type: "reopen", target_id: "uuid-123", target_title: "写周报" }],
    };
  ...
  it("reopen action 的 type 字段值是字符串 'reopen'，不是其他已有 type", () => {
    const result = {
      actions: [{ type: "reopen", target_id: "uuid-456" }],
    };
    const actions = parseActions(result, "");
...
describe("AC-2: parseActions 正确解析 reopen action", () => {
  it("reopen action：正确提取 target_id", () => {
    const result = {
      actions: [{ type: "reopen", target_id: "uuid-reopen-001", target_title: "已完成的任务" }],
    };
    const actions = parseActions(result, "");
  ...
  it("reopen action：正确提取 target_title", () => {
    const result = {
      actions: [{ type: "reopen", target_id: "uuid-reopen-002", target_title: "写周报" }],
    };
    const actions = parseActions(result, "");
  ...
  it("reopen action：target_id 缺失时不强制填充", () => {
    const result = {
      actions: [{ type: "reopen", target_title: "某个任务" }],
    };
    const actions = parseActions(result, "");
    expect(actions[0].type).toBe("reopen");
  ...
  it("reopen action：target_title 缺失时不强制填充", () => {
    const result = {
      actions: [{ type: "reopen", target_id: "uuid-only" }],
    };
    const actions = parseActions(result, "");
    expect(actions[0].type).toBe("reopen");
  ...
  it("reopen action 不应带 tasks 字段（不是 create）", () => {
    const result = {
      actions: [{ type: "reopen", target_id: "uuid-001", target_title: "写周报" }],
    };
    const actions = parseActions(result, "");
  ...
  it("reopen action 不应带 changes 字段（不是 update）", () => {
    const result = {
      actions: [{ type: "reopen", target_id: "uuid-001" }],
    };
    const actions = parseActions(result, "");
