...
describe("buildCompressedSpaceText", () => {
  ...
  it("输出包含空间标题", () => {
    ...
    const result = buildCompressedSpaceText("项目Alpha", [task], [], today, nameMap, 5000);
  ...
  it("输出包含统计信息", () => {
    ...
    const result = buildCompressedSpaceText("项目Beta", tasks, [], today, nameMap, 5000);
    // 应包含总数或完成率之类的统计
      ...
      for (let i = 0; i < 200; i++) {
        tasks.push(
          makeTask({
            ...
            status: i % 10 === 0 ? 2 : 0,
            completed_at: i % 10 === 0 ? "2026-03-19T08:00:00Z" : undefined,
          })
        );
      }
  ...
  it("空数据不会崩溃", () => {
    const result = buildCompressedSpaceText("空项目", [], [], today, nameMap, 5000);
  ...
  it("只有 1 个任务且无日志时正常工作", () => {
    ...
    const result = buildCompressedSpaceText("小项目", [task], [], today, nameMap, 5000);
  ...
  it("输出不超过 maxChars 限制", () => {
    ...
    const maxChars = 3000;
    const result = buildCompressedSpaceText("限制测试", tasks, logs, today, nameMap, maxChars);
