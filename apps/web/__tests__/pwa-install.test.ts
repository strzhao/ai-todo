...
describe("PWA install banner logic", () => {
  ...
  describe("iOS detection regex", () => {
    ...
    it("should detect iPod", () => {
      expect(iosRegex.test("Mozilla/5.0 (iPod touch; CPU iPhone OS 17_0 like Mac OS X)")).toBe(
        true
      );
    });
    ...
    it("should not match desktop Chrome", () => {
      expect(iosRegex.test("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Chrome/120")).toBe(
        false
      );
    });
