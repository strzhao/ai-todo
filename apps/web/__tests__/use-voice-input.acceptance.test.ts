...
describe("iOS Safari detection", () => {
  ...
  it("detects iPad user agent", () => {
    expect(isIOSSafari("Mozilla/5.0 (iPad; CPU OS 17_0 like Mac OS X) AppleWebKit/605.1.15")).toBe(
      true
    );
  });

  it("does not match Android", () => {
    expect(isIOSSafari("Mozilla/5.0 (Linux; Android 14; Pixel 8) AppleWebKit/537.36")).toBe(false);
  });
