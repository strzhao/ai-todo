...
describe("AC-1: 任务相关通知点击不跳转，改为抽屉展示", () => {
  ...
  it("点击有 task_id 的通知时应触发 onOpenDetail 回调", async () => {
    ...
    const n = makeNotification({ type: "task_assigned", task_id: "t-1" });
    const { container } = render(<NotificationItem notification={n} onOpenDetail={onOpenDetail} />);
...
function setupRouteMocks(overrides: {
  ...
  vi.doMock("@/lib/auth", () => ({
    getUserFromRequest: vi
      .fn()
      .mockImplementation(overrides.getUserFromRequest ?? (() => Promise.resolve(null))),
  }));
  vi.doMock("@/lib/db", () => ({
    initDb: vi.fn(),
    getTaskForUser: vi
      .fn()
      .mockImplementation(overrides.getTaskForUser ?? (() => Promise.resolve(null))),
    completeTask: vi.fn(),
    reopenTask: vi.fn(),
    deleteTask: vi.fn(),
    updateTask: vi.fn(),
