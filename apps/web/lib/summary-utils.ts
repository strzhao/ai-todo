...
function buildFullTreeText(
  ...
  const children = allTasks.filter(
    (t) => t.parent_id === parentId || (t.space_id === parentId && !t.parent_id)
  );
  return children
    .map((t) => {
      const line =
        formatTaskLine(t, indent, nameMap, allTasks) +
...
export function buildCompressedSpaceText(
  ...
  const todayCompleted = tasks.filter((t) => t.completed_at?.slice(0, 10) === date).length;
...
export function buildFilteredTaskTree(
  ...
  for (const t of relevant) {
    ...
    const childText = buildFilteredTaskTree(allTasks, t.id, indent + 1, nameMap, relevantIds);
