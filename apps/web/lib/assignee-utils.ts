...
export function addRecentAssignee(spaceId: string, email: string): void {
  ...
  try {
    ...
    localStorage.setItem(`assignee_recent_${spaceId}`, JSON.stringify(updated));
  } catch {
...
export function sortMembers<T extends SortableMember>(
  ...
  // Start with "未指派"
  const items: Array<T & { email: string }> = [{ email: "" } as T & { email: string }];
  ...
  const remaining = activeMembers
    ...
    .sort((a, b) => getDisplayLabel(a.email, a).localeCompare(getDisplayLabel(b.email, b)));
