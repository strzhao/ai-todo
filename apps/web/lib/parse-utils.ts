...
export function parseItem(
  ...
  return {
    ...
    priority:
      typeof item.priority === "number" && [0, 1, 2, 3].includes(item.priority)
    ...
    ...(Array.isArray(item.mentions) && item.mentions.length > 0
      ...
      : {}),
    ...(typeof item.milestone === "string" && item.milestone
      ? { milestone: item.milestone.slice(0, 100) }
      : {}),
  };
}
    ...
    return (result.actions as Record<string, unknown>[]).map((a): ParsedAction => {
      ...
      if (type === "update" && a.changes && typeof a.changes === "object") {
        ...
        action.changes = {
          ...
          ...(typeof c.priority === "number" && [0, 1, 2, 3].includes(c.priority)
            ? { priority: c.priority as 0 | 1 | 2 | 3 }
            : {}),
          ...
          ...(typeof c.progress === "number" && c.progress >= 0 && c.progress <= 100
            ? { progress: c.progress }
            : {}),
          ...(c.milestone !== undefined
            ? { milestone: c.milestone ? String(c.milestone).slice(0, 100) : null }
            : {}),
        };
      }
