...
export async function POST(req: NextRequest) {
  ...
  if (file.size > MAX_FILE_SIZE) {
    return NextResponse.json({ error: "音频文件过大（上限 4MB）" }, { status: 400 });
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: "语音识别服务异常" }, { status: 502 });
    }
    ...
    return NextResponse.json({ error: "语音识别服务不可用" }, { status: 503 });
  }
}
