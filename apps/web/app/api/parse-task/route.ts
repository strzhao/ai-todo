      ...
      "tasks": [
        {
          ...
          "assignee": "被指派人邮箱（可选）",
          "mentions": ["邮箱"]（可选）,
          "milestone": "里程碑名称（如 'v1.0 发布'、'Sprint 1 完成'），非里程碑为null",
...
- 【里程碑】：当用户说"这是里程碑"、"标记为里程碑"、"设为milestone"时，设置 milestone 字段为用户指定的名称；未指定名称时用任务标题作为里程碑名称
...
- 自引用：用户说"给我/我来/我负责/帮我/分配给我"等表达时，assignee 填当前用户邮箱
...
export async function POST(req: NextRequest) {
  ...
  const memberKey = (members ?? [])
    .map((m) => `${m.email}|${m.display_name ?? ""}`)
    .sort()
    .join(",");
  const taskKey = (tasksCtx ?? [])
    .map((t) => t.id)
    .sort()
    .join(",");
  ...
  try {
    ...
    const result = await rt.track("llm", async () =>
      client.chatJson(
        [
          { role: "system", content: SYSTEM_PROMPT },
          {
            role: "user",
            content: `当前时间: ${now}（用户时区: ${timezone}，当地时间: ${new Date(now).toLocaleString("zh-CN", { timeZone: timezone, hour12: false })}）\n\n当前用户: ${user.email}${membersContext}${tasksContext}${parentContext}\n\n用户输入: ${text}`,
          },
        ],
        0.1
      )
    ...
    if (err instanceof LLMError) {
      ...
      return rt.json(
        {
          error: isTimeout ? "AI 解析超时，请稍后重试" : "AI 解析失败，请手动填写",
