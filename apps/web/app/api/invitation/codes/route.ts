...
async function fetchCodes(cookie: string) {
  const res = await fetch(`${AUTH_ISSUER}/api/auth/invitation-codes?serviceKey=${SERVICE_KEY}`, {
    headers: { cookie },
    cache: "no-store",
  });
...
export async function GET(req: NextRequest) {
  ...
  const filtered = (data.codes as { status: string }[]).filter((c) => c.status !== "REVOKED");
