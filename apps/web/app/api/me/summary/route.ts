...
import { getPersonalDaySummaryData, hasPersonalDayContent } from "@/lib/daily-digest";
  ...
  if (data.logs.length > 0) {
    sections.push(
      `\n## 今日进展日志（${data.logs.length} 条）\n${data.logs
        .map((l) => `- 任务「${l.task_title}」: ${l.content.slice(0, 120)}`)
        .join("\n")}`
    );
  }
...
export async function GET(req: NextRequest) {
  ...
  const date =
    dateParam && DATE_RE.test(dateParam) ? dateParam : new Date().toISOString().slice(0, 10);
...
export async function POST(req: NextRequest) {
  ...
  const date =
    body.date && DATE_RE.test(body.date) ? body.date : new Date().toISOString().slice(0, 10);
  ...
  if (!hasPersonalDayContent(data)) {
    return NextResponse.json({ error: "当日没有任务活动" }, { status: 400 });
  }
