...
- 只基于提供的数据，不捏造，不给建议
- 简洁直接，聚类汇总
- 如果有关联空间数据，在相关部分中整合引用（如关联空间的进展、风险等），不要单独列一个"关联空间"章节
- 中文输出`;
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({
    config: config ? { ...config, data_sources: maskDataSources(config.data_sources) } : null,
    defaults: {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.data_sources) {
    ...
    const existingMap = new Map((existing?.data_sources ?? []).map((s) => [s.id, s]));
  ...
  return NextResponse.json({
    config: updated ? { ...updated, data_sources: maskDataSources(updated.data_sources) } : null,
  });
}
