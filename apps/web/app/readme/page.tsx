...
const cliExamples = [
  ...
  {
    cmd: "ai-todo notes:create --title 'WebSocket 替代轮询的想法' --tags '技术,优化'",
    desc: "创建笔记",
  },
    ...
    <div className="space-y-6">
      <Card className="border-blue-200 dark:border-blue-900 bg-blue-50/50 dark:bg-blue-950/20">
        <CardHeader>
          ...
          <CardDescription>
            ai-todo CLI 是为 AI agent 设计的命令行工具。所有输出为结构化
            JSON，命令从服务端动态发现， 旨在让 AI agent 直接管理任务，减少人的介入。人通过 AI agent
            间接使用此工具。
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <section className="space-y-3">
        ...
        <Card>
          <CardContent className="pt-6 space-y-2 text-sm">
            <p>
              CLI 的所有业务命令从服务端{" "}
              <code className="bg-muted px-1.5 py-0.5 rounded">/api/manifest</code> 动态获取，
      ...
      <section className="space-y-3">
        ...
        <Card>
          <CardContent className="pt-6 space-y-2 text-sm text-foreground/90">
            <p>AI agent（如 Claude Code）可以通过以下方式集成 ai-todo CLI：</p>
            <ul className="list-disc pl-5 space-y-1">
              <li>
                在 agent 的工具链中注册{" "}
                <code className="bg-muted px-1.5 py-0.5 rounded">ai-todo</code> 命令
              </li>
              <li>
                Agent 调用{" "}
                <code className="bg-muted px-1.5 py-0.5 rounded">ai-todo tasks:list</code>{" "}
                获取任务上下文
              </li>
            ...
            <p className="text-muted-foreground">
              输出格式：成功返回数据 JSON，错误返回{" "}
              <code className="bg-muted px-1.5 py-0.5 rounded">{`{"error": "...", "status": N}`}</code>
              ， 退出码 0=成功 1=错误 2=需登录。
            </p>
          </CardContent>
        </Card>
...
export default function ReadmePage() {
  return (
    <div className="app-content space-y-6">
      ...
      <ReadmeTabs cliContent={<CLIContent />} docContent={<DocContent />} />
    </div>
  );
}
