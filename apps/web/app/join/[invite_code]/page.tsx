...
export default function JoinSpacePage({ params }: Props) {
  ...
  useEffect(() => {
    params.then(({ invite_code }) => {
      setInviteCode(invite_code);
      fetch(`/api/spaces/join/${invite_code}`)
        .then(async (r) => {
          if (!r.ok) throw new Error("Invite link not found");
          return r.json() as Promise<SpacePreview>;
        })
        .then(setPreview)
  ...
  async function handleJoin() {
    ...
    setJoining(true);
    try {
      const res = await fetch(`/api/spaces/join/${inviteCode}`, { method: "POST" });
      if (!res.ok) {
      ...
      if (data.status === "active") {
        setStatus("joined");
        setTimeout(() => router.push(`/spaces/${data.space_id}`), 1500);
      } else {
        setStatus("pending");
      }
    } catch {
          ...
          preview && (
            <div className="border border-border rounded-xl p-6 space-y-5">
              <div className="text-center">
                <div className="w-16 h-16 rounded-2xl bg-primary/20 flex items-center justify-center text-primary font-bold text-3xl mx-auto mb-3">
                  {preview.name[0]?.toUpperCase()}
                </div>
                <h2 className="text-lg font-semibold">{preview.name}</h2>
                <p className="text-xs text-muted-foreground mt-1">
                  由 {preview.owner_email} 创建 · {preview.member_count} 名成员
                </p>
              </div>

              {status === "idle" && (
                <>
                  {preview.invite_mode === "approval" && (
              ...
              {status === "joined" && (
                <div className="text-center text-sm text-sage">✓ 加入成功！正在跳转...</div>
              )}
