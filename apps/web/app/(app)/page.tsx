...
export default function TaskHomePage() {
  ...
  const todayCount = useMemo(() => tasks.filter((t) => isDueToday(t.due_date)).length, [tasks]);
  ...
  function handleComplete(id: string) {
    ...
    mutateCurrent((prev) => prev?.filter((t) => t.id !== id && !descendantIds.has(t.id)), {
      revalidate: true,
    });
    if (done) {
      mutateCompleted((prev) => [{ ...done, status: 2 as const }, ...(prev ?? [])].slice(0, 20), {
        revalidate: true,
      });
    }
    window.dispatchEvent(new Event("tasks-changed"));
  ...
  function handleReopen(id: string) {
    ...
    mutateCompleted((prev) => prev?.filter((t) => t.id !== id), { revalidate: true });
    if (reopened) {
      mutateCurrent(
