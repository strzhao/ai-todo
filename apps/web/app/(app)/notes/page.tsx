...
export default function NotesPage() {
    ...
    try {
      const res = await fetch("/api/tasks", {
        ...
        body: JSON.stringify({
          ...
          tags,
          type: 1,
          ...(voiceRawText ? { voice_raw_text: voiceRawText } : {}),
        }),
      });
      if (res.ok) {
  ...
  function handleUpdate(id: string, updates: Partial<Task>) {
    mutateNotes((prev) => prev?.map((n) => (n.id === id ? { ...n, ...updates } : n)), {
      revalidate: true,
    });
  }

  function handleDelete(id: string) {
    mutateNotes((prev) => prev?.filter((n) => n.id !== id), { revalidate: true });
    window.dispatchEvent(new Event("tasks-changed"));
  }
    ...
    <div className="app-content">
      ...
      {isLoading ? (
        ...
        <EmptyState
          text="加载失败"
          subtext="请检查网络后重试"
          action={{ label: "重试", onClick: () => mutateTasks() }}
        />
      ) : groups.length === 0 ? (
        <EmptyState
