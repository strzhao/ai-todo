...
function getHostname(url: string): string {
  try {
    ...
    const path =
      pathname.length > 1 ? pathname.slice(0, 20) + (pathname.length > 20 ? "..." : "") : "";
...
function LinkChip({ url }: { url: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <path
          d="M3.5 1.5H1.5v9h9v-2M7 1.5h3.5V5M5 7l5.5-5.5"
          strokeLinecap="round"
...
export function RichText({ text, truncate, className }: RichTextProps) {
  ...
  if (parts.length === 1 && parts[0].type === "text") {
    return (
      <span
        className={`${truncate ? "truncate block" : "whitespace-pre-wrap break-words"} ${className ?? ""}`}
      >
        {text}
      </span>
    );
