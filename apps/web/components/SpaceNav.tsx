...
export function SpaceNav({ spaces, orgs, userEmail, userNickname, isDev }: Props) {
  ...
  const fetchSpaceTasks = useCallback((spaceId: string) => {
    ...
    fetch(`/api/tasks?space_id=${spaceId}`)
      ...
      .catch(() => {})
      .finally(() => {
        inflightRef.current.delete(spaceId);
      });
  }, []);
        ...
        <div key={node.id} className="space-y-0.5">
          <div className="flex items-center gap-1 min-w-0">
            ...
            <Link
              ...
              className={`min-w-0 flex-1 py-1 px-2 rounded text-xs transition-colors flex items-center gap-1 ${
                isFocused
                  ? "bg-primary/10 text-primary font-medium"
            ...
            >
              ...
              {node.assignee_email &&
                (() => {
                  const member = spaceMembersMap[spaceId]?.find(
                    (m) => m.email === node.assignee_email
                  );
                  ...
                  return hasNickname ? (
                    <span
                      className="text-[9px] text-muted-foreground shrink-0 max-w-[4rem] truncate"
                      title={node.assignee_email}
                    >
                      {label}
                    </span>
                  ) : (
                    <span
                      className="w-3.5 h-3.5 rounded-full bg-primary/20 text-primary text-[9px] flex items-center justify-center shrink-0 font-medium"
                      title={node.assignee_email}
                    >
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <nav
        className="hidden md:flex flex-col h-screen border-r border-border/60 bg-background fixed left-0 top-0 pt-6 pb-4 px-3 z-10"
        style={{ width: "var(--sidebar-width, 208px)" }}
      >
        ...
        <div className="flex-1 space-y-0.5 overflow-y-auto">
          <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider px-3 mb-1">
            个人
          </p>
          <Link href="/" className={navLinkCls(isTasksHome)}>
            任务
          </Link>
          <Link href="/notes" className={navLinkCls(isActive("/notes"))}>
            笔记
          </Link>
          ...
          <div className="pt-4">
            <div className="flex items-center justify-between px-3 mb-1">
              ...
              >
                <span
                  className={`text-[8px] transition-transform ${orgsCollapsed ? "-rotate-90" : ""}`}
                >
                  ▼
                </span>
                团队组织
              ...
              return (
                <div key={space.id}>
                  <div className="group/space relative">
                    ...
                    >
                      <span
                        className={`text-[10px] text-muted-foreground shrink-0 transition-transform ${isExpanded ? "" : "-rotate-90"}`}
                      >
                        ▼
                      </span>
                      ...
                      {(space.task_count ?? 0) > 0 && (
                        <span className="ml-auto text-[10px] text-muted-foreground group-hover/space:hidden">
                          {space.task_count}
                        </span>
                      )}
                    </Link>
                    ...
                    >
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          setOpenMenuSpaceId((v) => (v === space.id ? null : space.id));
                        }}
        ...
        <div className="px-3 pt-4 border-t border-border/60 space-y-1">
          <Link
            href="/notifications"
            className={`${navLinkCls(pathname === "/notifications")} relative`}
          >
            通知
            {unreadNotifCount > 0 && (
          ...
          <Link
            href="/account"
            className={`${navLinkCls(pathname.startsWith("/account"))} relative`}
          >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-background border-t border-border/60 z-10 flex">
        <Link
          href="/"
          className={`flex-1 flex flex-col items-center py-3 text-xs gap-1 ${isTasksHome ? "text-primary" : "text-muted-foreground"}`}
        >
          <span className="text-base">📋</span>任务
        </Link>
        <Link
          href="/notes"
          className={`flex-1 flex flex-col items-center py-3 text-xs gap-1 ${pathname === "/notes" ? "text-primary" : "text-muted-foreground"}`}
        >
          <span className="text-base">📝</span>笔记
        </Link>
        <Link
          href="/spaces"
          className={`flex-1 flex flex-col items-center py-3 text-xs gap-1 ${pathname.startsWith("/spaces") ? "text-primary" : "text-muted-foreground"}`}
        >
          <span className="text-base">👥</span>空间
        </Link>
        <Link
          href="/orgs"
          className={`flex-1 flex flex-col items-center py-3 text-xs gap-1 ${pathname.startsWith("/orgs") ? "text-primary" : "text-muted-foreground"}`}
        >
          <span className="text-base">🏢</span>组织
        </Link>
        <Link
          href="/notifications"
          className={`flex-1 flex flex-col items-center py-3 text-xs gap-1 relative ${pathname === "/notifications" ? "text-primary" : "text-muted-foreground"}`}
        >
        ...
        <Link
          href="/account"
          className={`flex-1 flex flex-col items-center py-3 text-xs gap-1 ${pathname.startsWith("/account") ? "text-primary" : "text-muted-foreground"}`}
        >
