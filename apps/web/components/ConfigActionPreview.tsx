...
function ActionRow({
  ...
  action: ParsedSummaryConfigAction;
  onToggleSaveMode?: (mode: "update" | "save_as") => void;
}) {
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            <span>
              {isReset
                ? "恢复系统 Prompt 为默认"
                : `修改系统 Prompt：${action.prompt_changes_description ?? "已更新"}`}
            </span>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            <span>
              {isReset
                ? "恢复数据模板为默认"
                : `修改数据模板：${action.template_changes_description ?? "已更新"}`}
            </span>
      ...
      <div className="flex items-start gap-2 text-sm">
        ...
        <span>
          ...
          <span className="text-muted-foreground ml-1">
            {ds.method ?? "GET"}{" "}
            {ds.url ? (ds.url.length > 50 ? ds.url.slice(0, 50) + "..." : ds.url) : ""}
          </span>
        </span>
      </div>
    );
  ...
  if (action.type === "toggle_datasource") {
    return (
      <div className="flex items-start gap-2 text-sm">
        <span className="text-info flex-shrink-0">⇄</span>
        <span>
          {action.enabled ? "启用" : "禁用"}数据源「
          <span className="font-medium">{action.datasource_name ?? "未知"}</span>」
        </span>
      </div>
    );
  }
        ...
        <div className="text-sm space-y-1">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <span>
                另存为新模板「
                <span className="font-medium">{action.template.name ?? "未命名"}</span>」
              </span>
          ...
          <div className="flex gap-1.5 ml-5">
            <button
              ...
              className={`text-[10px] px-2 py-0.5 rounded transition-colors ${
                action.type === "update_prompt_template"
                  ? "bg-sage text-white"
...
function applyActions(
  ...
  for (const action of actions) {
    switch (action.type) {
      case "update_prompt":
        systemPrompt =
          action.new_prompt === null || action.new_prompt === undefined ? null : action.new_prompt;
        break;

      case "update_template":
        dataTemplate =
          action.new_template === null || action.new_template === undefined
            ? null
            : action.new_template;
        break;
      ...
      case "add_datasource":
        if (action.datasource) {
          ...
          const existingIdx = dataSources.findIndex((s) => s.url === action.datasource!.url);
          ...
          if (existingIdx >= 0) {
            dataSources[existingIdx] = {
              ...dataSources[existingIdx],
              ...newDs,
              id: dataSources[existingIdx].id,
            };
          } else {
...
export function ConfigActionPreview({
  actions,
  currentConfig,
  defaults,
  spaceId,
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium">AI 理解：{localActions.length} 项配置变更</p>
      </div>
      ...
      <div className="space-y-2 py-1">
        {localActions.map((action, i) => (
          <ActionRow
            ...
            onToggleSaveMode={
              action.type === "update_prompt_template" ||
              (action.type === "add_prompt_template" &&
                action._originalType === "update_prompt_template")
                ? (mode) => {
