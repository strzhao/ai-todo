    ...
    <div className="rounded-xl border border-border/70 bg-background px-3 py-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {item.meta && <p className="mt-1 text-xs text-muted-foreground leading-5">{item.meta}</p>}
          {item.excerpt && (
  ...
  if (!digest) {
    return (
      <div className="px-1 py-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground/70">这是旧版每日摘要通知，未保存结构化明细。</p>
      </div>
    );
  }
        ...
        <section key={section.key} className="space-y-3">
          <div className="flex items-baseline justify-between gap-3">
            ...
            {section.overflow_count > 0 && (
              <span className="text-xs text-muted-foreground">另 {section.overflow_count} 项</span>
            )}
          </div>
