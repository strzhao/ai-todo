...
function resolveCreateParentTask(
  parsed: Omit<ParsedTask, "children">,
  allTasks: Task[],
  parentTaskId?: string
): Task | null | undefined {
...
export function MultiTaskPreview({
  tasks,
  raw,
  onConfirm,
  onCancel,
  ...
  if (
    tasks.length === 1 &&
    !tasks[0].children?.length &&
    !tasks[0].parent_target_id &&
    !tasks[0].parent_target_title
  ) {
    return (
      <ParsePreviewCard
        parsed={tasks[0]}
...
function MultiTaskList({
  ...
  function updateTitle(localId: string, title: string) {
    setFlatItems((prev) =>
      prev.map((i) => (i.localId === localId ? { ...i, task: { ...i.task, title } } : i))
    );
  }

  function getParentBadge(
    item: FlatItem
  ): { label: string; unresolved: boolean; full: string } | null {
    if (item.level === 1) {
  ...
  const unresolvedParentTargetCount = flatItems
    ...
    .filter(
      (i) =>
        !parentTaskId &&
        !!(i.task.parent_target_id || i.task.parent_target_title) &&
        !resolveCreateParentTask(i.task, allTasks, parentTaskId)
    ).length;
