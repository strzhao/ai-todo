    ...
    try {
      ...
      const res = await fetch("/api/parse-task", {
        ...
        body: JSON.stringify({
          ...
          ...(parentTaskId
            ? { parent_task: { id: parentTaskId, title: parentTaskTitle ?? "" } }
            : {}),
        }),
      });
      ...
      // 兼容旧格式
      const actions: ParsedAction[] = data.actions ?? [
        { type: "create", tasks: (data.tasks ?? []) as import("@/lib/types").ParsedTask[] },
      ];
            ...
            >
              ...
              <span className="flex-1 min-w-0">
                {(m.display_name || m.nickname) && (
                  <span className="font-medium mr-1">{m.display_name || m.nickname}</span>
                )}
