...
function SettingsCard({ title, children }: { title?: string; children: React.ReactNode }) {
  return (
    <section>
      {title && <h2 className="text-xs font-medium text-muted-foreground mb-2 px-1">{title}</h2>}
...
function SettingsRow({
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-sm flex-1 min-w-0 ${destructive ? "text-destructive" : "text-foreground"}`}
      >
        {label}
      </span>
...
const SwitchIcon = (
  ...
  >
    <path
      fillRule="evenodd"
      d="M8 1a.75.75 0 0 1 .75.75v1.69l1.22-1.22a.75.75 0 1 1 1.06 1.06L8.75 5.56V8a.75.75 0 0 1-1.5 0V5.56L4.97 3.28a.75.75 0 0 1 1.06-1.06l1.22 1.22V1.75A.75.75 0 0 1 8 1ZM4.25 8.5a3.75 3.75 0 1 0 7.5 0h1.5a5.25 5.25 0 1 1-10.5 0h1.5Z"
      clipRule="evenodd"
    />
  </svg>
);
...
export function AccountContent({ userEmail, userNickname, isDev }: Props) {
  ...
  function updateCache(
    allCodes: InvitationCode[],
    q: { used: number; total: number },
    total: number,
    more: boolean
  ) {
    try {
      localStorage.setItem(
        "invite-codes-cache",
        JSON.stringify({ codes: allCodes, quota: q, total, hasMore: more })
      );
    } catch {
      /* quota exceeded */
    }
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        {!inviteLoading &&
          ...
          codes.map((c) => (
            <SettingsRow
              ...
              label={
                <span
                  className={`font-mono tracking-wider ${c.status === "REDEEMED" ? "line-through text-muted-foreground" : ""}`}
                >
                  {c.code}
                </span>
              }
      ...
      <SettingsCard title="账号操作">
        <SettingsRow icon={SwitchIcon} label="切换账号" onClick={handleSwitchAccount} destructive />
      </SettingsCard>
    </div>
  );
}
