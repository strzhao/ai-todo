...
function resolveCreateParentTask(
  parsed: ParsedTask,
  allTasks: Task[],
  parentTaskId?: string
): Task | null | undefined {
...
function getCreateParentDisplay(
  parsed: ParsedTask,
  allTasks: Task[],
  parentTaskId?: string,
  spaceId?: string
): { status: "matched" | "unresolved"; title: string } | null {
  if (parentTaskId) {
...
function ActionRow({
  ...
  if (action.type === "create") {
    ...
    const parentInfo = first
      ? getCreateParentDisplay(first, allTasks, parentTaskId, spaceId)
      : null;
        ...
        <span>
          {count === 1 && first ? (
            <>
              ...
              {parentInfo?.status === "matched" && (
                <>
                  ，在「<span className="font-medium">{parentInfo.title}</span>」下
                </>
              )}
              {first.priority !== undefined && first.priority !== 2
                ? ` · ${PRIORITY_LABELS[first.priority]}`
                : ""}
              {first.due_date ? ` · ${formatDateTime(first.due_date)}截止` : ""}
              {parentInfo?.status === "unresolved" && (
                <span className="text-xs ml-1 text-destructive">
                  （父任务未匹配：{parentInfo.title}）
                </span>
              )}
            </>
          ) : (
            <>
              ...
              {matchedCount > 0 && (
                <span className="text-xs text-muted-foreground ml-1">
                  （{matchedCount} 个已匹配父任务）
                </span>
              )}
              {unresolvedCount > 0 && (
                <span className="text-xs ml-1 text-destructive">
                  （{unresolvedCount} 个父任务未匹配）
                </span>
              )}
            </>
          )}
  ...
  if (action.type === "update" && action.changes) {
    ...
    if (c.assignee_email !== undefined) {
      ...
      changeParts.push(
        `经办人→${c.assignee_email ? getDisplayLabel(c.assignee_email, am) : "未指派"}`
      );
    }
    ...
    return (
      <div className={`flex items-start gap-2 text-sm ${notFound ? "text-destructive" : ""}`}>
        ...
        <span>
          更新「<span className="font-medium">{displayTitle}</span>」：
          {changeParts.join("、") || "无变更"}
          {notFound && <span className="text-xs ml-1 text-destructive">（未找到匹配任务）</span>}
        </span>
      </div>
    );
  ...
  if (action.type === "move") {
    ...
    return (
      <div
        className={`flex items-start gap-2 text-sm ${notFound || invalidSame ? "text-destructive" : ""}`}
      >
        <span className="flex-shrink-0 text-info">↳</span>
        <span>
          移动「<span className="font-medium">{displayTitle}</span>」到「
          <span className="font-medium">{parentDisplayTitle}</span>」下
          {notFound && <span className="text-xs ml-1 text-destructive">（未找到匹配任务）</span>}
          {invalidSame && (
            <span className="text-xs ml-1 text-destructive">（目标父任务不能是自身）</span>
          )}
        </span>
      </div>
    );
...
export function ActionPreview({
  ...
  const [permissionErrors, setPermissionErrors] = useState<
    Array<{ action: string; taskTitle: string; error: string }>
  >([]);
      ...
      try {
          ...
          const parentResults = await Promise.all(
            parents.map((t) => {
              ...
              const body = {
                ...
                ...(resolvedParent && resolvedParent.id !== spaceId
        ...
        if (action.type === "update") {
          ...
          if (res.ok) {
            ...
            result.errors = [
              ...(result.errors ?? []),
              { action: "update", taskTitle: task.title, error: err.error },
            ];
          }
        }
        ...
        if (action.type === "move") {
          ...
          if (res.ok) {
            ...
            result.errors = [
              ...(result.errors ?? []),
              { action: "move", taskTitle: task.title, error: err.error },
            ];
          }
        }
        ...
        if (action.type === "complete") {
          ...
          if (res.ok) {
            ...
            result.errors = [
              ...(result.errors ?? []),
              { action: "complete", taskTitle: task.title, error: err.error },
            ];
          }
        }
        ...
        if (action.type === "delete") {
          ...
          if (res.ok || res.status === 204) {
            ...
            result.errors = [
              ...(result.errors ?? []),
              { action: "delete", taskTitle: task.title, error: err.error },
            ];
          }
        }
        ...
        if (action.type === "reopen") {
          ...
          if (res.ok) {
            ...
            result.errors = [
              ...(result.errors ?? []),
              { action: "reopen", taskTitle: task.title, error: err.error },
            ];
          }
        }
      } catch (err) {
  ...
  const hasUnresolvable =
    ...
    createActions.some((a) =>
      (a.tasks ?? []).some(
        (t) =>
          !parentTaskId &&
          !!(t.parent_target_id || t.parent_target_title) &&
          !resolveCreateParentTask(t, allTasks, parentTaskId)
      )
    );
  const executableCount = actions.reduce((n, a) => {
    if (a.type === "create") {
      const creatable = (a.tasks ?? []).filter(
        (t) => resolveCreateParentTask(t, allTasks, parentTaskId) !== null
      ).length;
      return n + creatable;
    }
    const task = resolveTask(a, allTasks);
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium">AI 理解：{actions.length} 项操作</p>
        {hasUnresolvable && <span className="text-xs text-destructive">部分任务未找到</span>}
      </div>
      ...
      <div className="flex gap-2 justify-end pt-1">
        ...
        <Button size="sm" onClick={handleConfirm} disabled={executing || executableCount === 0}>
          {executing
            ? "执行中..."
            : `全部执行${executableCount > 0 ? ` (${executableCount})` : ""}`}
        </Button>
      </div>
    </div>
  );
