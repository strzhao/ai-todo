...
export function AssigneePicker({
  ...
  // ─── Derived display ────────────────────────────────────────────────
  const currentMember = currentEmail ? members.find((m) => m.email === currentEmail) : undefined;
  const currentLabel = currentEmail ? getDisplayLabel(currentEmail, currentMember) : null;
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    if (isDown) {
      ...
      e.preventDefault();
      setHighlightIdx((prev) => (prev <= 0 ? filteredItems.length - 1 : prev - 1));
    }
  }
  ...
  if (readonly) {
    if (variant === "detail") {
      return <span className="text-sm text-foreground">{currentLabel ?? "未指派"}</span>;
    }
  ...
  const searchInput = (
    <input
      ...
      className={`w-full px-3 py-1.5 text-xs bg-transparent outline-none placeholder:text-muted-foreground/40 ${
        openAbove ? "border-t border-border/50" : "border-b border-border/50"
      }`}
    />
  );
  ...
  const memberList = (
    <div className="max-h-[180px] overflow-y-auto py-0.5">
      {filteredItems.map((item, idx) => (
        <button
          ...
          className={`w-full flex items-center gap-2 text-left px-3 py-1.5 text-xs transition-colors ${
            item.isCurrent ? "font-medium text-sage bg-muted/50" : "text-foreground"
      ...
      {filteredItems.length === 0 && (
        <div className="px-3 py-1.5 text-xs text-muted-foreground/50">无匹配成员</div>
      )}
    </div>
  );
  ...
  const panel = open ? (
    <div
      ...
      } ${openAbove ? "bottom-full mb-1" : "top-full mt-1"} left-0`}
    >
      {openAbove ? (
        <>
          {memberList}
