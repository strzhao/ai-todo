...
export const TaskItem = memo(function TaskItem({
  ...
  const hasSubtasks =
    (subtasks?.length ?? 0) > 0 || (onDrillDown && (childCountMap?.[task.id] ?? 0) > 0);
  const subtaskCount = onDrillDown
    ? (childCountMap?.[task.id] ?? subtasks?.length ?? 0)
    : (subtasks?.length ?? 0);
  ...
  useEffect(() => {
    setMilestoneInput(task.milestone ?? "");
  }, [task.milestone]);
  ...
  useEffect(() => {
    ...
    function handler(e: MouseEvent) {
      ...
      if (priorityOpen && priorityRef.current && !priorityRef.current.contains(target))
  ...
  const isDueToday =
    highlightTodayDue && task.due_date ? isTodayFn(new Date(task.due_date)) : false;
  ...
  return (
    <div
      id={`task-${task.id}`}
      ref={rowRef}
      className={`border-b last:border-0 border-border/50 ${completing ? "opacity-40" : ""} ${isFocused ? "animate-focus-flash" : ""}`}
    >
      {/* Main task row */}
      <div
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-1.5 mt-4 flex-wrap items-center min-h-[22px]">
                ...
                <div className="absolute left-0 top-full mt-1 z-50 bg-background border border-border rounded-md shadow-lg py-1 flex flex-col w-max">
                  {([0, 1, 2, 3] as const).map((v) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`inline-flex items-center justify-center w-5 h-4 rounded text-[10px] font-medium ${opt.cls}`}
                        >
                          {opt.label}
                        </span>
            ...
            {isDueToday && <span className="today-task-pill">今日优先</span>}
            ...
            {localTags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-0.5 text-xs text-muted-foreground group/tag"
              >
                #{tag}
                <button
            ...
            {(localProgress > 0 || task.status !== 2) && (
              <div
                className="inline-flex items-center gap-1 group/progress"
                title={`进度 ${localProgress}%`}
              >
            ...
            <div ref={milestoneRef} className="relative">
              {milestoneEditing ? (
                ...
                >
                  <span className={task.milestone ? "" : "grayscale opacity-50"}>🚩</span>
                  {task.milestone
                    ? ` ${task.milestone.length > 8 ? task.milestone.slice(0, 8) + "…" : task.milestone}`
                    : ""}
                </button>
              )}
            </div>
