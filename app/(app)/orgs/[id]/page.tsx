...
export default function OrgDetailPage({ params }: Props) {
  ...
  const [org, setOrg] = useState<(Organization & { my_role?: string; my_user_id?: string }) | null>(
    null
  );
  ...
  async function saveSettings() {
    ...
    setSaving(false);
    setOrg((prev) =>
      prev ? { ...prev, name: name.trim(), description: description.trim() || undefined } : prev
    );
  }
        ...
        <div className="space-y-2">
          {spaces.length === 0 ? (
            ...
            spaces.map((space) => (
              <div
                key={space.id}
                className="flex items-center gap-3 p-4 border border-border/60 rounded-lg"
              >
      ...
      {tab === "settings" && (
        <div className="space-y-8">
          ...
          <section className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">通过链接可直接加入组织</p>
          </section>
          ...
          {isOwner && (
            <section className="space-y-3 border-t border-destructive/20 pt-6">
              ...
              <p className="text-xs text-muted-foreground">
                解散组织后，关联的空间不会被删除，但会解除与组织的关联。
              </p>
              <div className="space-y-2">
                <label className="text-xs text-muted-foreground">
                  输入组织名称确认：<strong>{org.name}</strong>
                </label>
                <div className="flex gap-2">
                  <Input
