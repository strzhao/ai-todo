...
## 今日进展
先用一句话概括项目整体进展（含关键数字：总任务数、已完成、完成率），然后按重要性列出今日有实际更新的任务（基于今日进展日志）。每条包含：
- 任务名称（父任务 › 子任务）
- 具体进展
- 负责人（如有）

如果今日无进展日志，仍需输出整体概述一句话，然后说明"今日暂无具体进展更新"。
...
## 后续安排
基于任务数据中尚未开始或进度较低的任务，列出接下来需要推进的事项：
- 按优先级排列
- 标注截止日（如有）
- 只列出任务数据中实际存在的待办项，不要自行编造或给建议

如果所有任务都已完成或在进行中且进度良好，说明"当前无待推进事项"。
